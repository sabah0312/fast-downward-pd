...
static shared_ptr<SearchAlgorithm> parse_cmd_line_aux(const vector<string> &args) {
 ...
 shared_ptr<DeorderAlgorithm> parse_cmd_line_for_deorder(int argc, const char **argv) {
    vector<string> args;
    for (int i = 1; i < argc; ++i) {
            string arg = argv[i];
            args.push_back(arg);
        }
    string domain_filepath = args[0];
    string problem_filepath = args[1];
    string plan_filepath = args[2];
    args.erase(args.begin(), args.begin() + 3);


    using DeorderPtr = shared_ptr<DeorderAlgorithm>;
    DeorderPtr deorder_algorithm = nullptr;
    // TODO: Remove code duplication.
    for (size_t i = 0; i < args.size(); ++i) {
        const string &arg = args[i];
        bool is_last = (i == args.size() - 1);
        if (arg == "--deorder") {
            ...
            ++i;
            const string &deorder_arg = args[i];
            try {
                parser::TokenStream tokens = parser::split_tokens(deorder_arg);
                parser::ASTNodePtr parsed = parser::parse(tokens);
                parser::DecoratedASTNodePtr decorated = parsed->decorate();
                plugins::Any constructed = decorated->construct();
                deorder_algorithm = plugins::any_cast<DeorderPtr>(constructed);
            } catch (const plugins::BadAnyCast &) {
                input_error("Could not interpret the argument of --search as a search algorithm.");
            } catch (const utils::ContextError &e) {
                input_error(e.get_message());
    ...
    if (deorder_algorithm) {
        DeorderManager &deorderManager = deorder_algorithm->get_deorder_manager();
        deorderManager.set_plan_file_path(plan_filepath);
        deorderManager.set_domain_file_path(domain_filepath);
        deorderManager.set_problem_file_path(problem_filepath);
    }
    return deorder_algorithm;
}
